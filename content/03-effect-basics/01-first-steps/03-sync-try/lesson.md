...
右のコードを、まず書き換えずに Run してみてください。`実行された!` が `--- まだ何も実行されていない ---` より**先に**出力されてしまいます。JavaScript は引数を即座に評価するので、`Effect.succeed(console.log(...))` と書いた時点で `console.log` が走ってしまうのです。これでは設計図になりません。
