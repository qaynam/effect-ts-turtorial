...
右のコードの `parseAge` は、入力文字列を年齢に変換します。失敗パターンは 2 つあります。数値として読めない場合と、負の数の場合です。ところが返り値が `Option<number>` なので、どちらの失敗も同じ `none` に潰れてしまい、Run すると 2 行とも「理由は不明」になります。Option は「無い」ことは言えても、「**なぜ**無いのか」を運ぶ場所を持っていないのです。
