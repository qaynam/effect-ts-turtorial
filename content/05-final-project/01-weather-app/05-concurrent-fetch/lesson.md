...
都市名の配列を `map(fetchCity)` に通して「1 行を作る Effect」の配列を作り、`Effect.all` が同時に最大 3 件ずつ実行して、結果を元の順序のまま配列で返します。リストに混ぜた Ryugujo(竜宮城)は実在しないので、前レッスンで作った `CityNotFoundError` の経路が一覧の中で生きてきます。
