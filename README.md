...
- **レッスン**: `content/<part>/<lesson>/` に `lesson.md`(frontmatter + 日本語本文)、
  `initial.ts`、`solution.ts` を置き、`content/curriculum.json` に 1 行足すだけで追加できる
- **進捗**: localStorage(zustand + persist)。サーバー不要の純粋な SPA
