...
## カリキュラム(全 5 Part・55 レッスン)

1. **FP の基礎**(10) — 純粋関数・副作用・アクション/計算/データ・イミュータビリティ・高階関数・クロージャ・カリー化・部分適用・pipe / flow
2. **FP の理論**(12) — ADT・パターンマッチ・Option / Either の自作・Functor / Monad / Applicative と各種法則
3. **Effect 入門**(12) — Effect 型・succeed / fail・sync / try・promise・実行・map / flatMap・Effect.gen・2 種類のエラー・タグ付きエラー・catch・match
4. **Effect 実践**(14) — Service / Layer・Scope・acquireRelease・Fiber・Effect.all・race / timeout・retry / repeat・Ref・Schema・HttpClient
