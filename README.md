...
OG 画像と meta タグは絶対 URL をビルド時に埋め込むため、公開先の URL が必要。

```sh
cp .env.example .env    # SITE_URL に公開先の URL を書く(.env は git 管理外)
pnpm deploy
```

`SITE_URL` が未設定・localhost・非 https のときは `pnpm deploy` が止まる
(壊れた OG に気づかないままデプロイしないため)。
...
- **OGP**: レッスンは静的に確定しているので、OG 画像(satori + resvg)とレッスンごとの
  meta 入り HTML を**ビルド時に生成**して静的配信する。クローラは JS を実行しないため
  SPA のままでは meta が届かないが、この方式なら Worker を書かずにページ別 OG が出る
